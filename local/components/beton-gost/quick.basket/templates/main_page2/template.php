...
<?
  ...
  function getClassToColumnTable($index, $parentId, $subIndex = 0) {
    $subIndexesArray = [
      271 => 1
    ];
    if (array_key_exists($parentId, $subIndexesArray)) {
      $subIndex = $subIndexesArray[$parentId];
    }
    $index -= $subIndex;
    if ($index < 0) $index = 0;
    return 'a1a01__row_item_' . $index;
  }
...
<div class="maxwidth-theme quick_basket__container">
    <div class="quick_basket__title">
        Актуальный прайс на продукцию бетонного завода
    </div>
</div>
<?
?>
<section class="quick_basket maxwidth-theme swipeignore w100">
    ...
    <div class="quick__basket_list">
        ...
        <? foreach ($arChilds as $parentId => $childs) { 
          ...
          ?>
            <div class="tab__item <?php if (isset($arParams['ON_MAIN']) && $arParams['ON_MAIN'] === 'Y') :?>toogle<?php endif;?>" tabindex="<?=$parentId?>">
              <?
              // if ($USER->IsAdmin()){
              //   echo "<pre>";
              //   var_dump($childs);
              ...
              ?>
                <? foreach ($childs as $id => $child) { 
                    ...
                    //   echo "<pre>";
                    //   var_dump($child["ID"]);
                    //   var_dump($arItemsSort[$child["ID"]]);
                    //   echo "</pre>";
                    // }

                  ?>
                    <? //if ($arItemsSort[$child["ID"]]) { ?>
                    <?php if (isset($arParams['ON_MAIN']) && $arParams['ON_MAIN'] === 'Y') :?>
                      <section class="toggle">
                    <?php endif; ?>
                        <label class="table_title" data-id="<?php echo $child["ID"];?>"><?=$child["NAME"]?></label>
                        <?
                        if (!isset($arResult["arSections3"][$child["ID"]])){

                        ?>
                        <div class="table_container <?php if (isset($arParams['ON_MAIN']) && $arParams['ON_MAIN'] === 'Y') :?>toggle-content<?php endif;?><?php if ($child["ID"] == $asphaltId): ?> asphalt-container<?php endif;?>">
                            <div class="table__header">
                                <? if ($headerTableItems[$arSectionsId[$parentId]]) { ?>
                                    <? foreach ($headerTableItems[$arSectionsId[$parentId]] as $key => $itemName) { ?>
                                        <?php if ($child["ID"] == $asphaltId && $key == 0) continue;?>
                                        <div class="a1a01__row_item <?=getClassToColumnTable($key, $child['ID'])?>"><?=$itemName?></div>
                                    <? } ?>
                                <? } else { ?>
                            ...
                            <? foreach ($arItemsSort[$child["ID"]] as $id => $item) { ?>
                                ...
                                <? $minPrice = number_format($item["MIN_PRICE"]["VALUE"], 0, ',', ' '); ?>
                                ...
                                <div class="a1a01__row" data-measure="<?=$itemMeasure?>" data-itemId="<?=$item['ID']?>" data-id="<?=$minOfferPriceID?>">
                                    <?php if ($child["ID"] != $asphaltId):?>
                                    <div class="a1a01__row_item <?=getClassToColumnTable(0, $child['ID']);?>">
                                        <? if ($markId) { ?>
                                    ...
                                    <?php endif;?>
                                    <div class="a1a01__row_item <?=getClassToColumnTable(1, $child['ID']);?>">
                                        <a href="<?=$item["DETAIL_PAGE_URL"]?>" class="product__link" target="_blank"><?=$item["NAME"]?></a>
                                    </div>
                                    <? if ($minPrice) { ?>
                                        <div class="a1a01__row_item item__price <?=getClassToColumnTable(2, $child['ID']);?>"><?=$minPrice?> &#8381;</div>
                                    <? } ?>
                                    <div class="a1a01__row_item <?=getClassToColumnTable(3, $child['ID']);?>">
                                    ...
                                    <div class="a1a01__row_item <?=getClassToColumnTable(4, $child['ID']);?>">
                    ...
                    // 3 уровень - начало
                      if (isset($arResult["arSections3"][$child["ID"]])){
                        foreach ($arResult["arSections3"][$child["ID"]]['CHILD'] as $sec_id=>$val){
                          ?>
                    <?php if (isset($arParams['ON_MAIN']) && $arParams['ON_MAIN'] === 'Y') :?>
                      <section class="toggle">
                    <?php endif; ?>
                          <label class="table_title" style="background: grey;"><?=$val["NAME"]?></label>

                          <div class="table_container <?php if (isset($arParams['ON_MAIN']) && $arParams['ON_MAIN'] === 'Y') :?>toggle-content<?php endif;?>">
                              <div class="table__header">
                                  <? if ($headerTableItems[$arSectionsId[$parentId]]) { ?>
                                      <? foreach ($headerTableItems[$arSectionsId[$parentId]] as $key => $itemName) { ?>
                                          <?php if ($child["ID"] == $asphaltId && $key == 0) continue;?>
                              ...
                              //var_dump($arResult["arItems3_ids"]);
                              foreach ($arResult["arItems3_ids"] as $id => $item) { 

                                // var_dump($sec_id);
                                // var_dump($item['IBLOCK_SECTION_ID']);
                                if ($sec_id != $item['IBLOCK_SECTION_ID']){
                                   continue;
                                }
                                ?>
                                <? //$minPrice = $item["MIN_PRICE"]["PRINT_VALUE"]; ?>
                    ...
                    <?php endif;?>
                        ...
                        // echo "<pre>";
                        //   var_dump($arResult["arSections3"][$child["ID"]]);
                        // echo "</pre>";
                      }
                      // 3 уровень - конец
                    ?>
    ...
    <div>
        <div class="quick__basket_form">
            <div class="form_orderxx">
                ...
                <div class="form__footer">
                    <button class="form__footer_basket btn btn-default btn-lg has-ripple"><span>В корзину</span>
                        <span class="form__footer_basket_icon"><img src="<?=$templateFolder?>/img/trash.svg" alt=""></span>
                    </button>
            ...
            <?/*<noindex>
                <article class="video-stream">
                    <h3 class="video-stream__title" style="margin-top:30px;">Трансляция камеры с завода в #REGION_NAME_DECLINE_PP#</h3>
                    <div class="iv-embed" style="padding:0;border:0;">
                        <div class="iv-v" style="display:block;margin:0;padding:1px;border:0;background:#000;"><iframe class="iv-i" style="" src="https://open.ivideon.com/embed/v2/?server=100-0GtTNrCo90n4QIhbOuuxVo&amp;camera=0&amp;width=&amp;height=&amp;lang=ru" width="320" height="180" frameborder="0" allowfullscreen></iframe></div>
        ...
        <div class="priceContainer">
            <div class='priceDownload'>
                <div class='headerPriceDownload'>
                    <b>Скачайте полный прайс-лист <?=date("Y")?> года с ценами</b>
                </div>
    ...
    <div class="float__form">
        ...
        <div class="float__form_btnWrap">
            <button class="float__form_basket"><span>В корзину</span>
                <span class="float__form_basket_icon"><img src="<?=$templateFolder?>/img/trash-red.svg" alt="trash"></span>
            </button>
        </div>
    </div>
</section>
