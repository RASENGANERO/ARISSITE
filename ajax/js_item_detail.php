...
<?if($arPost["PARAMS"]):?>
	...
	else
	{
		...
		foreach($arResult["ITEMS"] as $key => $arOffer)
		{
			...
			$result = false;
			$minPrice = 0;
			$bPriceVat = $arPost['PARAMS']['PRICE_VAT_INCLUDE'] === true || $arPost['PARAMS']['PRICE_VAT_INCLUDE'] === 'Y' || $arPost['PARAMS']['PRICE_VAT_INCLUDE'] === 'true';
	...
	<script>
		...
		setBuyBlock = function(th, obj, index)
		{
			...
			if(th.closest('.list').length) // is list view
			{
				...
				{
					let woc = inner.find('.wrapp-one-click');
					if(woc.length) {
						woc.replaceWith(obj.ONE_CLICK_BUY_HTML);
					} else {
