...
Asset::getInstance()->addJs(SITE_TEMPLATE_PATH."/vendor/js/velocity/velocity.js");
Asset::getInstance()->addJs(SITE_TEMPLATE_PATH."/vendor/js/velocity/velocity.ui.js");
Asset::getInstance()->addJs(SITE_TEMPLATE_PATH."/vendor/js/jquery.appear.js");
Asset::getInstance()->addJs(SITE_TEMPLATE_PATH."/js/speed.js");
...
<head itemscope itemtype="http://schema.org/WPHeader">
	...
	<link rel="stylesheet" href="<?=SITE_TEMPLATE_PATH?>/css/shvp.css?v=shvp">
...
<body class="<?=($bIndexBot ? "wbot" : "");?> site_<?=SITE_ID?> <?=($bIncludedModule ? CMax::getCurrentBodyClass() : '')?>" id="main" data-site="<?=SITE_DIR?>">
	...
	<?
        if($APPLICATION->GetCurDir() == "/dostavka/" || strpos($APPLICATION->GetCurDir(), "/price/") !== false  || $APPLICATION->GetCurDir() == "/sale/" || $APPLICATION->GetCurDir() == "/help/" || $APPLICATION->GetCurDir() == "/info/"){
            $dopFront = true;
        }else{
            $dopFront = false;
        }

        if(stripos($APPLICATION->GetCurDir(), "/laboratoriya/") !== false || $APPLICATION->GetCurDir() == "/info/marki-betona/" || $APPLICATION->GetCurDir() == "/info/klassy-betona/" || $APPLICATION->GetCurDir() == "/info/faq/" || $APPLICATION->GetCurDir() == "/company/" || $APPLICATION->GetCurDir() == "/primenenie/" || $APPLICATION->GetCurDir() == "/info/partner/"){
            $dopRightBlock = true;
        }else{
            $dopRightBlock = false;
        }
    ?>
    <div class="wrapper1 <?if($dopFront){echo "front_page";}?> <?if($dopRightBlock){echo "rightDop";}?> <?=($isIndex && $isShowIndexLeftBlock ? "with_left_block" : "");?> <?=CMax::getCurrentPageClass();?> <?$APPLICATION->AddBufferContent(array('CMax', 'getCurrentThemeClasses'))?>  ">
		...
		<div class="wraps hover_<?=$arTheme["HOVER_TYPE_IMG"]["VALUE"];?>" id="content">
			<? //echo(str_replace('//', '/', $_SERVER['DOCUMENT_ROOT'].'/'.SITE_DIR.'include/header_include/top_wraps.php'));?>
			...
			<?elseif(!$isWidePage):?>
				<div class="wrapper_inner <?if($dopFront){echo "front";}?> <?=($isHideLeftBlock ? "wide_page" : "");?> <?=$APPLICATION->ShowViewContent('wrapper_inner_class')?>">
			<?endif;?>
