...
<script type="text/javascript">
	...
	ymaps.ready(function (){
		...
		$sebekon_jq_delivery("#map<?=$arResult['ID']?>").height(400);
		...
		deliveryObj.map.controls.remove('geolocationControl');
deliveryObj.map.controls.remove('searchControl');
deliveryObj.map.controls.remove('trafficControl');
deliveryObj.map.controls.remove('typeSelector');
//deliveryObj.map.controls.remove('fullscreenControl');
deliveryObj.map.controls.remove('rulerControl');
		...
		//var shifted = deliveryObj['delivers'];
		
		$sebekon_jq_delivery('#soa-property-7').val(deliveryObj.coordsName);


		$sebekon_jq_delivery('#bx-soa-delivery .bx-soa-section-title-container').append($sebekon_jq_delivery('#SEBEKON_DELIVERYPRICE_ORDER'));
		$sebekon_jq_delivery('#bx-soa-delivery, #bx-soa-delivery .bx-soa-pp-company-description').css('width', '100%');
		$sebekon_jq_delivery('#bx-soa-delivery, #bx-soa-delivery .bx-soa-pp-company-description').css('marginRight', '0px');
		$sebekon_jq_delivery('#SEBEKON_DELIVERYPRICE_ORDER').modal('hide');
		$sebekon_jq_delivery('#SEBEKON_DELIVERYPRICE_ORDER').css('position', 'static');
		$sebekon_jq_delivery('#SEBEKON_DELIVERYPRICE_ORDER').fadeIn('slow');
		$sebekon_jq_delivery('.sebekon_delivery_price_link').prop('onclick', null).off('click');

		if (!window.isYmaps) {
			$sebekon_jq_delivery('.sebekon_delivery_price_link').trigger('click');
			window.isYmaps = true;
		}
	});
...
<button onclick="submitParentForm(); return false;" class="btn btn-default pull-right sebekon_select_btn" style="display: none;"><?=GetMessage('sebekon_DELIVERY_SELECT')?></button>
<form id="DP_search_form<?=$arResult['ID']?>" class="DP_search_form" action="" method="POST">
    ...
    <input type="submit" value="<?=GetMessage('sebekon_SEARCH_BTN')?>" class="btn btn-default" style="padding: 10px; margin-left: 10px; text-transform: lowercase;" />
	<p class="help"><i><?=GetMessage('sebekon_SEARCH_HELP')?></i></p>
</form>
<div id="map<?=$arResult['ID']?>"></div>
