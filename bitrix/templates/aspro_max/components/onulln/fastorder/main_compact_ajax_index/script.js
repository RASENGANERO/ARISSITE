...
console.log('444');
	...
	$(input).closest('.bx_filter_block').find('input[type=checkbox], input[type=radio]').each(function(){
		...
		if(i != 0){
			...
			all_text=$(input).closest('.bx_filter_parameters_box').find('.bx_filter_parameters_box_container input:checked').data('name');
			$(input).closest('.bx_filter_parameters_box').find('.bx_filter_parameters_box_title .count_selected').text(all_text);
			$(input).closest('.bx_filter_parameters_box').find('.fo_param_name').hide();
		}
		else{
...
JCSmartFilter.prototype.postHandler = function (result, fromCache)
{
	...
	if (!!result && !!result.ITEMS)
	{
		...
		if (!!modef && !!modef_num)
		{
			fastOrderUrl = result.FILTER_AJAX_URL;
	...
	console.log('123ss');
			if($('.bx_filter_parameters_box.set').length == 0) {
				$('.bx_filter_parameters_box').each(function(){
					$(this).removeClass('fake-set');
					$(this).removeClass('set');
					$(this).find('.filter.label_block.radio').show();
					$(this).find('.filter.label_block.radio .disabled').parent().hide();
					if($(this).is('.set'))
						return;
					var all_text = '';
					$(this).find('.bx_filter_parameters_box_title .count_selected').text(all_text);
					$(this).find('.fo_param_name').show();
				});
				/*
					Возможно тут нужно добавить сборс параметров по умолчанию.
					Добавил.
				*/
								
				priceFOitem = result.FAST_ORDER.PRICE;
				idFOitem = result.FAST_ORDER.ID;
				getFOPrice(priceFOitem);
				getFullPrice();
			}
			else {
				$('.bx_filter_parameters_box').each(function(){
					...
					//var all_text = ': ';
					var all_text = '';
					$(this).find('.bx_filter_parameters_box_container input:not(.disabled)').each(function() {
						if(all_text == '')
							all_text = all_text + $(this).data('name');
						else 
							all_text = all_text + ', ' + $(this).data('name');
						countx++;
					});
					if(countx==2) {
						$(this).find('.fo_param_name').hide(); 
						$(this).find('.bx_filter_parameters_box_title .count_selected').text(all_text);
						$(this).addClass('fake-set set');
					} else {
						$(this).find('.fo_param_name').show();
						$(this).removeClass('fake-set');
						$(this).removeClass('set');
					...
					/*
					if(!$(this).is('.fo_prop__hide') && countx==2) {
						$(this).find('.bx_filter_parameters_box_title .count_selected').text(all_text);
						$(this).addClass('fake-set set');
					}*/
				});
			}
			console.log(result.FAST_ORDER.PRICE);
