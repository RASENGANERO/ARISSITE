...
<?if( count( $arResult["ITEMS"] ) >= 1 ){?>
	...
	<?}?>
		...
		<?foreach($arResult["ITEMS"] as $arItem){?>
			...
			if($arParams["SHOW_MEASURE"] == "Y" && isset($arItem["ITEM_MEASURE"]["TITLE"]) && $arItem["ITEM_MEASURE"]["TITLE"]){
				$strMeasure = $arItem["ITEM_MEASURE"]["TITLE"];
			}
			if(!$arItem["OFFERS"] || $arParams['TYPE_SKU'] !== 'TYPE_1'){
				if($arParams["SHOW_MEASURE"] == "Y" && $arItem["CATALOG_MEASURE"]){
					$arMeasure = CCatalogMeasure::getList(array(), array("ID" => $arItem["CATALOG_MEASURE"]), false, false, array())->GetNext();
					$strMeasure = $arMeasure["SYMBOL_RUS"] ?? $strMeasure;
				}
				if($bComplect){
			...
			elseif($arItem["OFFERS"]){
				$strMeasure = $arItem["MIN_PRICE"]["CATALOG_MEASURE_NAME"] ?? $strMeasure;
				if($arParams['TYPE_SKU'] == 'TYPE_1' && $arItem['OFFERS_PROP'])
				{
					...
					$strMeasure = $arCurrentSKU["MEASURE"] ?? $strMeasure;
			...
			<div class="list_item_wrapp item_wrap item item-parent clearfix bordered box-shadow">
				<div class="list_item item_info catalog-adaptive catalog-adaptive-mobile flexbox flexbox--row <?=($arItem["OFFERS"] ? 'has-sku' : '')?> <?=(!empty($arResult["DELIVERY_MAP_ID"])) ? 'catalog-adaptive-mobile-with-delivery-btn' : '';?>" id="<?=$arItemIDs["strMainID"];?>">
					...
					<div class="image_block">
						<div class="image_wrapper_block">
							...
							<?if($arParams['SHOW_GALLERY'] == 'Y'):?>
								...
								<?endif;?>
							<?else:?>
								<?\Helper\CHelperCommon::showImg($arParams, $arItem, false, true, '', 245, 184, 163, 123, 'w', ' sizes="(max-width: 420px) 163px, 245px" ');?>
							<?endif;?>
						</div>
					...
					<div class="description_wrapp">
						<div class="description">
							...
							<?if($arItem["PROPERTIES"]["TIZERS"]["VALUE"]) {?>
								<div class="tizers_block_list">
									<span class="muted777 font_xs" style="font-weight:bold;margin-bottom:10px;">Применение</span>
									<div class="items small-block  tops">
										<div class="row flexbox  ">
											...
											$arFilter = Array("IBLOCK_ID"=> 10, "ACTIVE"=>"Y", "ID"=> $arItem["PROPERTIES"]["TIZERS"]["VALUE"]);
													...
													<div class="item flexbox flexbox--row">
														<div class="image shine colored_theme_svg pull-left">
															<?if($arFields["PROPERTY_LINK_VALUE"]) {?>
																<a href="<?=$arFields["PROPERTY_LINK_VALUE"]?>" title="<?=$arFields["NAME"]?>">
																	<?
					...
					<div class="information_wrapp main_item_wrapper">
						<div class="information <?=($arItem["OFFERS"] && $arItem['OFFERS_PROP'] ? 'has_offer_prop' : '');?>  inner_content js_offers__<?=$arItem['ID'];?>_<?=$arParams["FILTER_HIT_PROP"]?>">
								...
								<?else:?>
									<?if(isset($arItem['PRICE_MATRIX']) && $arItem['PRICE_MATRIX']): // USE_PRICE_COUNT?>
										<?if(\CMax::GetFrontParametrValue('SHOW_POPUP_PRICE') == 'Y' || $arItem['ITEM_PRICE_MODE'] == 'Q' || (\CMax::GetFrontParametrValue('SHOW_POPUP_PRICE') != 'Y' && $arItem['ITEM_PRICE_MODE'] != 'Q' && count($arItem['PRICE_MATRIX']['COLS']) <= 1)):
											$arItem['CATALOG_MEASURE_NAME'] = $arItem['CATALOG_MEASURE_NAME'] ?? $strMeasure;?>
							...
							<?elseif($arItem["OFFERS"]):?>
								...
								<?}?>
							<?endif;?>
							<? if (!empty($arResult["DELIVERY_MAP_ID"])) : ?>
								<?$arParams['EXPRESSION_FOR_CALCULATE_DELIVERY'] = GetMessage('EXPRESSION_FOR_CALCULATE_DELIVERY');
								$arParams['DELIVERY_MAP_ID'] = $arResult["DELIVERY_MAP_ID"];
								$arParams['DELIVERY_BTN_CLASS'] = $arResult["DELIVERY_BTN_CLASS"];
								ob_start();
								\Helper\CHelperCommon::showCalculateDeliveryBlock($arItem['OFFERS'] ? $arItem["OFFERS"][$arItem["OFFERS_SELECTED"]]["ID"] : $arItem['ID'], $arParams);
								$deliveryBtn = ob_get_clean();?>
					...
					<?if(!$bComplect):?>
						...
						<?endif;?>
						<div class="fast_view_block-mobile rounded2 btn btn-xs font_upper_xs btn-transparent" data-event="jqm" data-param-form_id="fast_view" data-param-iblock_id="<?=$arParams["IBLOCK_ID"];?>" data-param-id="<?=$arItem["ID"];?>" data-param-item_href="<?=urlencode($arItem["DETAIL_PAGE_URL"]);?>" data-name="fast_view">
							<?=\CMax::showIconSvg("fw ncolor", SITE_TEMPLATE_PATH."/images/svg/quickview.svg", '', '', true, false);?><?=$fast_view_text;?>
						</div>
					<?endif;?>
