...
//if ($USER->IsAdmin()){
	foreach( $arResult["SECTIONS"] as $k=>$arItems ){
		...
		/*$arFilter = Array("IBLOCK_ID"=>$arItems['IBLOCK_ID'], "IBLOCK_SECTION_ID"=>$arItems['ID'], "INCLUDE_SUBSECTIONS" => 'Y', "ACTIVE_DATE"=>"Y", "ACTIVE"=>"Y", 'PROPERTY_LINK_REGION' => $arRegion['ID']);*/
		$arFilter = Array("IBLOCK_ID"=>$arItems['IBLOCK_ID'], "SECTION_ID"=>$arItems['ID'], "INCLUDE_SUBSECTIONS" => 'Y', "ACTIVE"=>"Y", 'PROPERTY_LINK_REGION' => $arRegion['ID']);
					...
					<div class="section-compact-list__item item bordered box-shadow flexbox flexbox--row" id="<?=$this->GetEditAreaId($arItems['ID']);?>">
						<?if($arParams["SHOW_SECTION_LIST_PICTURES"]=="Y"):?>
							<div class="section-compact-list__image<?=($bIcons && $arItems["UF_CATALOG_ICON"] ? ' with-icons colored_theme_svg' : '');?> flexbox flexbox--row">
								...
								<?\Helper\CHelperCommon::showSectionImg($arParams, $arItems, $bIcons);?>
							</div>
						<?endif;?>
