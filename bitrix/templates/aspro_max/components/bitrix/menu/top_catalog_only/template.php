...
<?if($arResult):?>
	<?if (!function_exists('showSubItemss')) {
		function showSubItemss($arParams = [
			...
			<?endif;?>
			<span class="name <?=$arParams['WIDE_MENU'] ? 'option-font-bold' : ''?>"><?=$arParams['ITEM']["TEXT"]?></span><?=($arParams['ITEM']["CHILD"] && $arParams['SHOW_CHILDS'] ? CMax::showSpriteIconSvg(SITE_TEMPLATE_PATH."/images/svg/trianglearrow_sprite.svg#trianglearrow_right", "right", ['WIDTH' => 3,'HEIGHT' => 5, 'INLINE' => 'N']) : '')?>
			<?if($arParams['ITEM']["LINK"]):?>
				</a>
			<?endif;?>
