				...
				<ul class="dropdown-menu toggle_menu">
					<?foreach($arParams['ITEM']["CHILD"] as $key => $arSubSubItem):?>
						...
						<li class="menu-item <?=(++$key > $arParams['VISIBLE_ITEMS_MENU'] ? 'collapsed' : '');?> <?=($arSubSubItem["CHILD"] && $arParams['SHOW_CHILDS'] ? "dropdown-submenu" : "")?> <?=($arSubSubItem["SELECTED"] ? "active" : "")?>">
							<a itemprop="url" href="<?=$arSubSubItem["LINK"]?>" title="<?=$arSubSubItem["TEXT"]?>"><span class="name"><?=$arSubSubItem["TEXT"]?></span></a>
			...
			<tr>
				<?foreach($arResult as $arItem):?>
					...
					<td class="menu-item unvisible <?=($arItem["CHILD"] ? "dropdown" : "")?> <?=(isset($arItem["PARAMS"]["CLASS"]) ? $arItem["PARAMS"]["CLASS"] : "");?>  <?=($arItem["SELECTED"] ? "active" : "")?>">
						<div class="wrap">
							<?php 
								if (!empty($arItem['PARAMS']['CUSTOM_URL'])) {
									$arItem["LINK"] = $arItem['PARAMS']['CUSTOM_URL'];
								}
							?>
							<a class="<?=($arItem["CHILD"] && $bShowChilds ? "dropdown-toggle" : "")?>" <?php echo (!empty($arItem['PARAMS']['NOFOLLOW']) && $arItem['PARAMS']['NOFOLLOW'] == 'Y') ? 'rel="nofollow"' : 'itemprop="url"';?> href="<?=$arItem["LINK"]?>">
								<div>
							...
							<?if($arItem["CHILD"] && $bShowChilds):?>
								...
								<div class="dropdown-menu <?=$bRightSide ? 'with_right_block' : ''?> <?=$arItem['bManyItemsMenu'] ? 'long-menu-items' : ''?> <?=(isset($arTheme['SHOW_RIGHT_SIDE']['DEPENDENT_PARAMS']['RIGHT_CONTENT']['VALUE']) ? $arTheme['SHOW_RIGHT_SIDE']['DEPENDENT_PARAMS']['RIGHT_CONTENT']['VALUE'] : $arTheme['RIGHT_CONTENT'])?>">
										...
										<ul class="menu-wrapper <?='menu-type-'.$MENU_TYPE?>" >
											<?foreach($arItem["CHILD"] as $arSubItem):?>
												<?if($MENU_TYPE == 2 && $bWideMenu):?>
													<?
													$bHasPicture = ( (isset($arSubItem['PARAMS']['PICTURE']) && $arSubItem['PARAMS']['PICTURE'] || (isset($arSubItem['PARAMS']['SECTION_ICON'])) ) && (isset($arTheme['SHOW_CATALOG_SECTIONS_ICONS']['VALUE']) ? $arTheme['SHOW_CATALOG_SECTIONS_ICONS']['VALUE'] : $arTheme['SHOW_CATALOG_SECTIONS_ICONS']) == 'Y');
														...
														<?if($bHasPicture && $bWideMenu):
															...
															if(is_array($arImg)):?>
																<div class="menu_img <?=$bIcon ? 'icon' : ''?>">
																	<a itemprop="url" href="<?=$arSubItem["LINK"]?>" class="noborder colored_theme_svg">
																		<?if(strpos($arImg["src"], ".svg") !== false):?>
																			<?=CMax::showIconSvg("cat_icons light-ignore", $arImg["src"]);?>
																		<?else:?>
															...
															<?endif;?>
														<?endif;?>
														<a itemprop="url" href="<?=$arSubItem["LINK"]?>" title="<?=$arSubItem["TEXT"]?>"><span class="name option-font-bold"><?=$arSubItem["TEXT"]?></span></a>
													</li>
												<?else: // type 1?>
													<?
													...
													<li class="<?=($arSubItem["CHILD"] && $bShowChilds ? "dropdown-submenu" : "")?> <?=$bIcon ? 'icon' : ''?> <?=($arSubItem["SELECTED"] ? "active" : "")?> <?=($bHasPicture ? "has_img" : "")?> parent-items">
														<?if($arItem['bManyItemsMenu']):?>
															<div class="flexbox flex-reverse">
																<?$bRightBanners = ($arSubItem["PARAMS"]["BANNERS"] && isset($arTheme['SHOW_RIGHT_SIDE']['DEPENDENT_PARAMS']['RIGHT_CONTENT']['VALUE']) && $arTheme['SHOW_RIGHT_SIDE']['DEPENDENT_PARAMS']['RIGHT_CONTENT']['VALUE'] == 'BANNER')?>
																<?$bRightBrands = ($arSubItem["PARAMS"]["BRANDS"] && $arSubItem["BRANDS"] && isset($arTheme['SHOW_RIGHT_SIDE']['DEPENDENT_PARAMS']['RIGHT_CONTENT']['VALUE']) && $arTheme['SHOW_RIGHT_SIDE']['DEPENDENT_PARAMS']['RIGHT_CONTENT']['VALUE'] == 'BRANDS')?>
																<?$bRightSide = (($bRightBanners || $bRightBrands) && isset($arTheme['SHOW_RIGHT_SIDE']['VALUE']) && $arTheme['SHOW_RIGHT_SIDE']['VALUE'] == 'Y');?>
																<?if($bRightSide):?>
																	<div class="right-side">
																		<div class="right-content">
																			<?if($bRightBanners):?>
																			...
																			<?elseif($bRightBrands):?>
																				<div class="brands-wrapper">
																					<?foreach ($arSubItem["BRANDS"] as $brand):?>
																						<div class="brand-wrapper">
																							<?if($brand['DETAIL_PAGE_URL']):?>
																								<a itemprop="url" href="<?=$brand['DETAIL_PAGE_URL']?>">
																							<?endif;?>
																								<img src="<?=CFile::GetPath($brand['PREVIEW_PICTURE'])?>" alt="<?=$brand['NAME']?>" title="<?=$brand['NAME']?>" />
																							<?if($brand['DETAIL_PAGE_URL']):?>
																								</a>
																...
																<div class="subitems-wrapper">
																	<ul class="menu-wrapper" >
																		<?foreach($arSubItem["CHILD"] as $arSubItem2):?>
																			<?
																			$bHasPicture = ( (isset($arSubItem2['PARAMS']['PICTURE']) && $arSubItem2['PARAMS']['PICTURE'] || (isset($arSubItem2['PARAMS']['SECTION_ICON'])) ) && isset($arTheme['SHOW_CATALOG_SECTIONS_ICONS']['VALUE']) && $arTheme['SHOW_CATALOG_SECTIONS_ICONS']['VALUE'] == 'Y');
