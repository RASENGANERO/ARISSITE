...
$strReturn = '<div class="breadcrumbs__item" id="bx_breadcrumb_00" itemprop="itemListElement" itemscope="" itemtype="http://schema.org/ListItem"><a class="breadcrumbs__link" href="/" title="Бетонный завод" itemprop="item"><span itemprop="name" class="breadcrumbs__item-name font_xs">Бетонный завод</span><meta itemprop="position" content="1"></a></div><span class="breadcrumbs__separator">&mdash;</span>';
if($arResult){
	...
	for($index = 0; $index < $cnt; ++$index){
		...
		if($arItem["LINK"] <> "" && $arItem['LINK'] != GetPagePath() && $arItem['LINK']."index.php" != GetPagePath() || $arSubSections){
			...
			if($arSubSections){
				...
				$strReturn .= '<span itemprop="name" class="breadcrumbs__item-name font_xs">'.$title.'</span><meta itemprop="position" content="'.($index + 2).'"></a>';
			}
			$strReturn .= '</div>';
		}
		else{
			/*$strReturn .= '<span class="breadcrumbs__item'.($bMobileBreadcrumbs ? ' breadcrumbs__item--mobile' : '').'" itemprop="itemListElement" itemscope itemtype="http://schema.org/ListItem"><link href="'.GetPagePath().'" itemprop="item" /><span><span itemprop="name" class="breadcrumbs__item-name font_xs">'.$title.'</span><meta itemprop="position" content="'.($index + 2).'"></span></span>';*/
			$strReturn .= '<span class="breadcrumbs__item'.($bMobileBreadcrumbs ? ' breadcrumbs__item--mobile' : '').'" itemprop="itemListElement" itemscope itemtype="http://schema.org/ListItem"><span><span itemprop="name" class="breadcrumbs__item-name font_xs">'.$title.'</span><meta itemprop="position" content="'.($index + 2).'"></span></span>';
		}
	}
