...
if ($section) {
    ...
    foreach (["UF_OGDESCRIPTION", "UF_OGTITLE"] as $prop){
        $section[$prop] = str_replace(array("#NAME#", "#SNAME#", "#REGION_NAME_DECLINE_TP#", "#REGION_NAME_DECLINE_PP#", "#REGION_NAME_DECLINE_RP#", "#REGION_NAME#"), array($section["NAME"], mb_strtolower($section["NAME"]), $arRegion["PROPERTY_REGION_NAME_DECLINE_TP_VALUE"], $arRegion["PROPERTY_REGION_NAME_DECLINE_PP_VALUE"], $arRegion["PROPERTY_REGION_NAME_DECLINE_RP_VALUE"], $arRegion["NAME"]), $section[$prop]);
    }

    $metaData = [];

    if(!empty($section["UF_OGDESCRIPTION"])){
        $metaData["og:description"] = $section["UF_OGDESCRIPTION"];
    }

    if(!empty($section["DETAIL_PICTURE"])){
        $ogimage = CFile::GetPath($section["DETAIL_PICTURE"]);
        $metaData["og:image"] = $ogimage;
    }

    if(!empty($section["UF_OGTITLE"])){
        $metaData["og:title"] = $section["UF_OGTITLE"];
    }
    ...
    $APPLICATION->SetPageProperty("CATALOG_SECTION_ID", $section["ID"]);
	...
	if(strlen($section["DESCRIPTION"])){
		$arSection["DESCRIPTION"] = \CMskBeton_Tools::ReplaceImgInContent($section["DESCRIPTION"], 410, 600);
	}
    ...
    $rsData = $entity_data_class::getList(array(
        "select" => array("*"),
        "order" => array("ID" => "ASC"),
        "filter" => array("UF_REGION" => $arRegion["ID"], "UF_SECTION" => $section["ID"])
    ));
    if($arData = $rsData->Fetch()){
        if(!empty($arData["UF_DESCRIPTION_SECTION"])){
            $arSection["DESCRIPTION"] = \CMskBeton_Tools::ReplaceImgInContent($arData["UF_DESCRIPTION_SECTION"], 410, 600);
        }
    }
  ...
  /*  CModule::IncludeModule("highloadblock");
    ...
    if($arData = $rsData->Fetch()){
        $title = str_replace(array("#NAME#", "#SNAME#", "#REGION_NAME_DECLINE_TP#", "#REGION_NAME_DECLINE_PP#", "#REGION_NAME_DECLINE_RP#", "#REGION_NAME#"), array($section["NAME"], mb_strtolower($section["NAME"]), $arRegion["PROPERTY_REGION_NAME_DECLINE_TP_VALUE"], $arRegion["PROPERTY_REGION_NAME_DECLINE_PP_VALUE"], $arRegion["PROPERTY_REGION_NAME_DECLINE_RP_VALUE"], $arRegion["NAME"]), $arData["UF_TITLE"]);
        $description = str_replace(array("#NAME#", "#SNAME#", "#REGION_NAME_DECLINE_TP#", "#REGION_NAME_DECLINE_PP#", "#REGION_NAME_DECLINE_RP#", "#REGION_NAME#"), array($section["NAME"], mb_strtolower($section["NAME"]), $arRegion["PROPERTY_REGION_NAME_DECLINE_TP_VALUE"], $arRegion["PROPERTY_REGION_NAME_DECLINE_PP_VALUE"], $arRegion["PROPERTY_REGION_NAME_DECLINE_RP_VALUE"], $arRegion["NAME"]), $arData["UF_DESCRIPTION"]);
        $keywords = str_replace(array("#NAME#", "#SNAME#", "#REGION_NAME_DECLINE_TP#", "#REGION_NAME_DECLINE_PP#", "#REGION_NAME_DECLINE_RP#", "#REGION_NAME#"), array($section["NAME"], mb_strtolower($section["NAME"]), $arRegion["PROPERTY_REGION_NAME_DECLINE_TP_VALUE"], $arRegion["PROPERTY_REGION_NAME_DECLINE_PP_VALUE"], $arRegion["PROPERTY_REGION_NAME_DECLINE_RP_VALUE"], $arRegion["NAME"]), $arData["UF_KEYWORDS"]);
        $APPLICATION->SetPageProperty('title', $title);
        $APPLICATION->SetPageProperty('description', $description);
        $APPLICATION->SetPageProperty('keywords', $keywords);

        if(!empty($arData["UF_OGTITLE_REGION"])){
            $metaData["og:title"] = str_replace(array("#NAME#", "#SNAME#", "#REGION_NAME_DECLINE_TP#", "#REGION_NAME_DECLINE_PP#", "#REGION_NAME_DECLINE_RP#", "#REGION_NAME#"), array($section["NAME"], mb_strtolower($section["NAME"]), $arRegion["PROPERTY_REGION_NAME_DECLINE_TP_VALUE"], $arRegion["PROPERTY_REGION_NAME_DECLINE_PP_VALUE"], $arRegion["PROPERTY_REGION_NAME_DECLINE_RP_VALUE"], $arRegion["NAME"]), $arData["UF_OGTITLE_REGION"]);
        }
        if(!empty($arData["UF_OGDESCRIPTION_REGION"])){
            $metaData["og:description"] = str_replace(array("#NAME#", "#SNAME#", "#REGION_NAME_DECLINE_TP#", "#REGION_NAME_DECLINE_PP#", "#REGION_NAME_DECLINE_RP#", "#REGION_NAME#"), array($section["NAME"], mb_strtolower($section["NAME"]), $arRegion["PROPERTY_REGION_NAME_DECLINE_TP_VALUE"], $arRegion["PROPERTY_REGION_NAME_DECLINE_PP_VALUE"], $arRegion["PROPERTY_REGION_NAME_DECLINE_RP_VALUE"], $arRegion["NAME"]), $arData["UF_OGDESCRIPTION_REGION"]);
        }
...
if (!empty($section['ID'])) $APPLICATION->SetPageProperty('sectionId', $section['ID']);
...
if (strpos($_SERVER['REQUEST_URI'], '/filter/') && isset($_SERVER['SCRIPT_URL'])) {
   	$resSotbit = CIBlockElement::GetList(
   		["ID"=>"ASC"], 
   		["IBLOCK_ID" => $arParams['LANDING_IBLOCK_ID'], "ACTIVE" => "Y", "PROPERTY_FILTER_URL" => $_SERVER['SCRIPT_URL']], 
   		false, 
   		[], 
   		['ID', 'TIMESTAMP_X', 'PROPERTY_OG_IMAGE']
   	);
   	if ($obSotbit = $resSotbit->GetNextElement()) {
   		$arFieldsSotbit = $obSotbit->GetFields();
   		$ipropValuesSotbit = new \Bitrix\Iblock\InheritedProperty\ElementValues($arParams['LANDING_IBLOCK_ID'], $arFieldsSotbit['ID']); 
		$arSeoPropsSotbit = $ipropValuesSotbit->getValues();
		$picture = CFile::GetFileArray($arFieldsSotbit['PROPERTY_OG_IMAGE_VALUE']);
		$APPLICATION->SetPageProperty('title', $arSeoPropsSotbit['ELEMENT_META_TITLE']);
		$APPLICATION->SetPageProperty('description', $arSeoPropsSotbit['ELEMENT_META_DESCRIPTION']);
		$APPLICATION->SetPageProperty('keywords', $arSeoPropsSotbit['ELEMENT_META_KEYWORDS']);
		$APPLICATION->SetTitle($arSeoPropsSotbit['ELEMENT_PAGE_TITLE']);
		if ($arFieldsSotbit['TIMESTAMP_X']) {
			CMax::AddMeta(
	            [
	            	'og:updated_time' => date(\DateTime::ATOM, MakeTimeStamp($arFieldsSotbit['TIMESTAMP_X']))
	            ]
	        );
		}
		...
		CMax::AddMeta(
            [
            	...
            	'og:image:alt' => $picture['DESCRIPTION'],
...
if (!$isSetMeta) {
	$rsSections = CIBlockSection::GetList([], $arSectionFilter, false, ['ID', 'IBLOCK_ID', 'PICTURE', 'DETAIL_PICTURE', 'TIMESTAMP_X']);
	if ($arSection = $rsSections->Fetch()) {
		
		if ($arSection['TIMESTAMP_X']) {
			CMax::AddMeta(
	            [
	            	'og:updated_time' => date(\DateTime::ATOM, MakeTimeStamp($arSection['TIMESTAMP_X']))
	            ]
	        );
		}
		
		if (!empty($arSection['PICTURE']) && !empty($arSection['DETAIL_PICTURE'])) {
...
$APPLICATION->SetPageProperty('is_catalog_section', true);