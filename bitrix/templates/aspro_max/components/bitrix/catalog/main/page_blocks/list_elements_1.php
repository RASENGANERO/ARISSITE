...
global $sotbit_meta;
global $sotbit_data;
$arSeoItem = [];	//	10.12.2024	Инфорормацию просили скрыть
?>
				...
				<?if(!$arSeoItem):?>
					<?if(
						...
						<?elseif($posSectionDescr=="BOTTOM"):?>
							<?
							if (isset($sotbit_data)){
								$meta = unserialize($sotbit_data['META']);

								//var_dump($meta);
							?>
								<div class="group_description_block bottom muted777">
									<div><?=$meta['ELEMENT_BOTTOM_DESC']?></div>
								</div>
						...
						<?
						$html = ob_get_clean();
						if (!\Helper\CHelperCommon::isFilterPage()) {
