	...
	foreach($productSlider as $i => $arImage){
		$arImage = array_merge(
			$arImage, array(
				...
				"SRCSET" => CHelperCommon::getSrcSet($arImage["ID"])
			)
		);
		if($pic = CMskBeton_Tools::GetWebPPictureFull($arImage["BIG"], true)) {
			$arPic = $pic['tmp_arr_src'];
			unset($pic['tmp_arr_src']);
			$arImage["BIG"] = $pic;
			foreach(array(
			...
			if($srcset = CHelperCommon::getSrcSet($arPic, 600, 450, 300, 250, 'w')) {
				$arImage['~SRCSET'] = $arImage['SRCSET'];
				$arImage['SRCSET'] = $srcset;
			}
		}
			...
			if($arOffer['PROPERTIES'] && isset($arOffer['PROPERTIES'][$arParams['ADDITIONAL_GALLERY_OFFERS_PROPERTY_CODE']]) && $arOffer['PROPERTIES'][$arParams['ADDITIONAL_GALLERY_OFFERS_PROPERTY_CODE']]['VALUE']){
				foreach($arOffer['PROPERTIES'][$arParams['ADDITIONAL_GALLERY_OFFERS_PROPERTY_CODE']]['VALUE'] as $img){
					...
					$arPhoto = array(
						...
						'TITLE' => $title,
						'ALT' => $alt,
						"SRCSET" => CHelperCommon::getSrcSet($img)
					);
					if('TYPE_1' === $arParams['TYPE_SKU']){
		...
		if($arOffer['MORE_PHOTO']){
			foreach($arOffer['MORE_PHOTO'] as $i => $arImage){
				if($arImage["ID"]){
					...
					$arOffer['MORE_PHOTO'][$i]["SRCSET"] = CHelperCommon::getSrcSet($arImage["ID"]);
				}
			}
		}
...
if(!empty($arResult['DISPLAY_PROPERTIES']))
{
	...
	{
		if (
			in_array($propKey, ['BRAND', 'CLASSES']) && 
			!empty($arDispProp['VALUE']) && 
			!empty($arDispProp['LINK_ELEMENT_VALUE']) && 
			!empty($arDispProp['LINK_ELEMENT_VALUE'][$arDispProp['VALUE']]['ID'])
		) {
			$res = CIBlockElement::GetByID($arDispProp['LINK_ELEMENT_VALUE'][$arDispProp['VALUE']]['ID']);
			if($obRes = $res->GetNextElement()) {
		  		$customLink = $obRes->GetProperty("CUSTOM_LINK");
			  	if ($customLink && !empty($customLink['~VALUE'])) {
			  		$arResult['DISPLAY_PROPERTIES'][$propKey]["DISPLAY_VALUE"] = '<a href="'.$customLink['~VALUE'].'">'.$arDispProp['LINK_ELEMENT_VALUE'][$arDispProp['VALUE']]['NAME'].'</a>';
			  	}
			}
		}
...
//		$arBrand['CATALOG_PAGE_URL'] = \Aspro\Max\Smartseo\General\Smartseo::replaceRealUrlByNew($arBrand['CATALOG_PAGE_URL']);
	...
	if($arBrand){
		...
		$res = CIBlockElement::GetByID($arBrand["ID"]);
		if($obRes = $res->GetNextElement()) {
	  		$customLink = $obRes->GetProperty("CUSTOM_LINK");
		  	if ($customLink && !empty($customLink['~VALUE'])) {
		  		$arBrand["DETAIL_PAGE_URL"] = $customLink['~VALUE'];
		  	}
		}
	}
}
...
$rsSections = CIBlockSection::GetList([], array('GLOBAL_ACTIVE' => 'Y', "ID" => $arResult["SECTION"]["ID"], "IBLOCK_ID" => $arParams["IBLOCK_ID"]), false, ['UF_DELIVERY_MAP_ID']);
...
$findme   = '#DIAGRAMM#';
$pos = strpos($arResult['DETAIL_TEXT'], $findme);

$arResult["NOINDEX"] = !empty($arResult["PROPERTIES"]["NOINDEX"]["VALUE"]) ? $arResult["PROPERTIES"]["NOINDEX"]["VALUE"] : 'N';
$arResult["FAQ_SECTIONS"] = !empty($arResult["PROPERTIES"]["FAQ_SECTIONS"]["VALUE"]) ? $arResult["PROPERTIES"]["FAQ_SECTIONS"]["VALUE"] : [];

if(
    $pos !== false &&
    !empty($arResult["PROPERTIES"]["DIAGRAMM_CATEGORY"]["VALUE"]) &&
    !empty($arResult["PROPERTIES"]["DIAGRAMM_VALUES"]["VALUE"]) &&
    !empty($arResult["PROPERTIES"]["DIAGRAMM_NAME"]["VALUE"])
) {
    $doubletext = explode($findme,$arResult['DETAIL_TEXT']);
    $arResult["DIAGRAMMA_ID"] = 'MAP_'.RandString();
    $arResult["DIAGRAMM_CATEGORY"] = $arResult["PROPERTIES"]["DIAGRAMM_CATEGORY"]["VALUE"];
    $arResult["DIAGRAMM_VALUES"] = $arResult["PROPERTIES"]["DIAGRAMM_VALUES"]["VALUE"];
    $arResult["DIAGRAMM_NAME"] = $arResult["PROPERTIES"]["DIAGRAMM_NAME"]["VALUE"];
    $arResult['DETAIL_TEXT'] = $doubletext[0].'<div id="'.$arResult["DIAGRAMMA_ID"].'" style="width: 100%; height: 300px;"></div>'.$doubletext[1];
    $this->__component->SetResultCacheKeys(array("DIAGRAMMA_ID", "DIAGRAMM_CATEGORY", "DIAGRAMM_VALUES", "DIAGRAMM_NAME", "DELIVERY_MAP_ID", "NOINDEX", "FAQ_SECTIONS"));
} else {
	$this->__component->SetResultCacheKeys(array("DIAGRAMMA_ID", "DIAGRAMM_CATEGORY", "DIAGRAMM_VALUES", "DIAGRAMM_NAME", "DELIVERY_MAP_ID", "NOINDEX", "FAQ_SECTIONS"));
}
...
//	echo "<pre>"; print_r($arResult["OFFERS"]); echo "</pre>";
    ...
    $arResult["CUSTOM_MIN_PRICE"] = $arResult["MIN_PRICE"]["VALUE_VAT"];
    if(!empty($arResult["OFFERS"])){
        foreach ($arResult["OFFERS"] as $OFFER){
            if(!empty($OFFER["MIN_PRICE"]["VALUE_VAT"]) && $arResult["CUSTOM_MIN_PRICE"] > $OFFER["MIN_PRICE"]["VALUE_VAT"]){
                $arResult["CUSTOM_MIN_PRICE"] = $OFFER["MIN_PRICE"]["VALUE_VAT"];
            }
        }
    }
    if(!empty($arResult["CUSTOM_MIN_PRICE"])){
        $arResult["MIN_PRICE"]["PRINT_VALUE_VAT"] =  CCurrencyLang::CurrencyFormat($arResult["CUSTOM_MIN_PRICE"],  $arResult["MIN_PRICE"]["CURRENCY"]);
	}
...
$arResult['DETAIL_TEXT'] = str_replace(array("#NAME#", "#SNAME#", "#PRICE#"), array($arResult["NAME"], mb_strtolower($arResult["NAME"]), $arResult["MIN_PRICE"]["PRINT_VALUE_VAT"]), $arResult['DETAIL_TEXT']);
$arResult['DETAIL_TEXT'] = \CMskBeton_Tools::ReplaceImgInContent($arResult['DETAIL_TEXT'], 410, 600);
