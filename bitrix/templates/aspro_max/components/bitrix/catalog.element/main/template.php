								...
								<div class="item-actions flexbox flexbox--row">
									...
									<div class="item-buttons item_<?=$arSKU["ID"]?>">
										...
										<?if(
											...
											&& !empty($arResult['DELIVERY_MAP_ID'])
										):?>
											<?=\Aspro\Functions\CAsproMaxCustom::showCalculateDeliveryBlock($arSKU['ID'], $arParams, $arParams['TYPE_SKU'] !== 'TYPE_1');?>
										<?endif;?>
...
<?//top info?>
<div class="product-info-wrapper" data-delivery-map-id="<?=(!empty($arResult['DELIVERY_MAP_ID'])) ? $arResult['DELIVERY_MAP_ID'] : '';?>">
		...
		<div class="page-menu-list-inner-wrapper">
			...
			<ul class="page-menu-list">
				<li>
					...
					<?php if (!empty($arResult['PROPERTIES']['PAGE_MENU_DESCRIPTION']['VALUE'])): ?>
					<ul class="page-sub-menu-list">
						<?php foreach ($arResult['PROPERTIES']['PAGE_MENU_DESCRIPTION']['VALUE'] as $key => $value): ?>
						<li><a href="#" data-box-id="<?php echo trim($arResult['PROPERTIES']['PAGE_MENU_DESCRIPTION']['DESCRIPTION'][$key]);?>" class="page-seo-menu-link"><?php echo trim($value);?></a></li>
						<?php endforeach;?>
					</ul>
					<?php endif;?>
				</li>
	...
	<div class="product-info <?=(!$showCustomOffer ? "noffer" : "");?> bordered rounded3" id="<?=$arItemIDs["strMainID"];?>">
							...
							<div itemscope itemtype="https://schema.org/ImageObject" id="photo-sku" class="product-detail-gallery__item product-detail-gallery__item--big text-center">
								<?if($arFirstPhoto["BIG"]["src"]):?>
									<a href="<?=($viewImgType=="POPUP" ? $arFirstPhoto["BIG"]["src"] : "javascript:void(0)");?>" <?=($bIsOneImage ? '' : 'data-fancybox="gallery"')?> class="product-detail-gallery__link <?=($viewImgType=="POPUP" ? "popup_link fancy" : "line_link fancy_zoom");?>" title="<?=$title;?>">
										<img id="<? echo $arItemIDs["ALL_ITEM_IDS"]['PICT']; ?>" class="lazy product-detail-gallery__picture <?=($viewImgType=="MAGNIFIER" ? "zoom_picture" : "");?>" <?php echo $arFirstPhoto["SRCSET"];?>  data-replace="<?=$arFirstPhoto["BIG"]["src"] ? $arFirstPhoto["BIG"]["src"] : $arFirstPhoto["SRC"]?>" src="<?=$arFirstPhoto["BIG"]["src"] ? $arFirstPhoto["BIG"]["src"] : $arFirstPhoto["SRC"]?>" <?=($viewImgType=="MAGNIFIER" ? 'data-xoriginal="'.$arFirstPhoto["BIG"]["src"].'" data-xpreview="'.$arFirstPhoto["THUMB"]["src"].'"' : "");?> alt="<?=$alt;?>" title="<?=$title;?>" decoding="async" itemprop="contentUrl" />
									</a>
								<?else:?>
									<img id="<? echo $arItemIDs["ALL_ITEM_IDS"]['PICT']; ?>" class="lazy product-detail-gallery__picture one" data-src="<?=$arFirstPhoto["SRC"]?>" data-replace="<?=$arFirstPhoto["SRC"]?>" src="<?=$arFirstPhoto["SRC"]?>" alt="<?=$alt;?>" title="<?=$title;?>" decoding="async" itemprop="contentUrl" />
								<?endif;?>
						...
						<?}//else{
							if($arResult["MORE_PHOTO"]){?>
								<?foreach($arResult["MORE_PHOTO"] as $i => $arImage){
									...
									$title=$arImage["TITLE"];
									?>
									<div itemscope itemtype="https://schema.org/ImageObject" id="photo-<?=$i?>" class="product-detail-gallery__item product-detail-gallery__item--big text-center">
										<?if(!$isEmpty){?>
											<a href="<?=($viewImgType=="POPUP" ? $arImage["BIG"]["src"] : "javascript:void(0)");?>" <?=($bIsOneImage ? '' : 'data-fancybox="gallery"')?> class="product-detail-gallery__link <?=($viewImgType=="POPUP" ? "popup_link fancy" : "line_link fancy_zoom");?>" title="<?=$title;?>">
												<img class="lazy product-detail-gallery__picture <?=($viewImgType=="MAGNIFIER" ? "zoom_picture" : "");?>" <?php echo $arImage["SRCSET"];?> data-replace="<?=$arImage["BIG"]["src"]?>" src="<?=$arImage["BIG"]["src"]?>" <?=($viewImgType=="MAGNIFIER" ? 'data-xoriginal="'.$arImage["BIG"]["src"].'" data-xpreview="'.$arImage["THUMB"]["src"].'"' : "");?> alt="<?=$alt;?>" title="<?=$title;?>"<?//=(!$i ? ' itemprop="image"' : '')?> decoding="async" itemprop="contentUrl" />
											</a>
										<?}else{?>
											<img class="lazy product-detail-gallery__picture one" data-replace="<?=$arImage["SRC"]?>" src="<?=$arImage["SRC"]?>" data-src="<?=$arImage["SRC"]?>" alt="<?=$alt;?>" title="<?=$title;?>" decoding="async" itemprop="contentUrl" />
										<?}?>
									</div>
								<?}?>
							<?}?>
								...
								<div itemscope itemtype="https://schema.org/ImageObject" id="photo-sku" class="product-detail-gallery__item product-detail-gallery__item--big text-center">
									<?if($arFirstPhoto["BIG"]["src"]):?>
										<a href="<?=$arFirstPhoto["BIG"]["src"];?>" data-fancybox="gallery" class="product-detail-gallery__link popup_link fancy" title="<?=$title;?>">
											<img id="<? echo $arItemIDs["ALL_ITEM_IDS"]['PICT']; ?>" class=" lazy product-detail-gallery__picture" <?php echo $arFirstPhoto["SRCSET"];?> src="<?=\Aspro\Functions\CAsproMax::showBlankImg($arFirstPhoto["BIG"]["src"])?>" alt="<?=$alt;?>" title="<?=$title;?>"<?//=(!$i ? ' itemprop="image"' : '')?> decoding="async" itemprop="contentUrl" />
										</a>
									<?else:?>
										<img id="<? echo $arItemIDs["ALL_ITEM_IDS"]['PICT']; ?>" class="lazy product-detail-gallery__picture" <?php echo $arFirstPhoto["SRCSET"];?> src="<?=$arFirstPhoto["SRC"]?>" alt="<?=$alt;?>" title="<?=$title;?>" decoding="async" itemprop="contentUrl" />
									<?endif;?>
								</div>
							<?}else{
								if($arResult["MORE_PHOTO"]){?>
										...
										<div itemscope itemtype="https://schema.org/ImageObject" id="photo-<?=$i?>" class="product-detail-gallery__item product-detail-gallery__item--big text-center">
											<?if(!$isEmpty){?>
												<a href="<?=$arImage["BIG"]["src"];?>" data-fancybox="gallery" class="product-detail-gallery__link popup_link fancy" title="<?=$title;?>">
													<img class="lazy product-detail-gallery__picture" <?php echo $arImage["SRCSET"];?> src="<?=\Aspro\Functions\CAsproMax::showBlankImg($arImage["BIG"]["src"])?>" alt="<?=$alt;?>" title="<?=$title;?>"<?//=(!$i ? ' itemprop="image"' : '')?> decoding="async" itemprop="contentUrl" />
												</a>
											<?}else{?>
												<img class="lazy product-detail-gallery__picture" <?php echo $arImage["SRCSET"];?> src="<?=$arImage["SRC"]?>" alt="<?=$alt;?>" title="<?=$title;?>" decoding="async" itemprop="contentUrl" />
											<?}?>
										</div>
									<?}?>
								<?}
							...
							<?if(true):?>
								<?if($countPhoto  > 1):?>
									<div class=" product-detail-gallery__slider owl-carousel owl-theme thmb<?=($bVertical ? ' product-detail-gallery__slider--vertical' : '');?>" data-size="<?=$countPhoto;?>" data-plugin-options='{"items": "4", "nav": true, "loop": false, "clickTo": ".product-detail-gallery__slider.big", "dots": false, "autoWidth": true, "margin": 10<?//if($bVertical):?>, "mouseDrag": false, "pullDrag": false<?//endif;?><?if($bMagnifier):?>, "magnifier": true<?endif;?>}' style="max-width:<?=ceil((($countPhoto <= 4 ? $countPhoto : 4) * 70) - 10)?>px;">
										<?if($arResult["MORE_PHOTO"]):?>
											<?foreach($arResult["MORE_PHOTO"] as $i => $arImage):?>
												<div id="photo-<?=$i?>" class="product-detail-gallery__item text-center  product-detail-gallery__item--thmb product-detail-gallery__item--thmb-bg" data-big="<?=$arImage["BIG"]["src"];?>" style="background: url(<?php echo (!$isEmpty) ? $arImage["THUMB"]["src"]: $arImage["SRC"];?>)">
													<?php if (false):?>
														<?if(!$isEmpty){?>
															<img class="lazy product-detail-gallery__picture" <?php echo $arImage["SRCSET"];?> src="<?=\Aspro\Functions\CAsproMax::showBlankImg($arImage["SMALL"]["src"])?>" alt="<?=$alt;?>" title="<?=$title;?>"/>
														<?}else{?>
															<img class="lazy product-detail-gallery__picture" <?php echo $arImage["SRCSET"];?> src="<?=$arImage["SRC"]?>" alt="<?=$alt;?>" title="<?=$title;?>" />
														<?}?>
													<?php endif; ?>
												</div>
											<?endforeach;?>
		...
		<div class="right_info">
			<div class="info_item hidden-md hidden-lg">
				...
				<?if(
					...
					<?ob_start()?>
						<?=\Aspro\Functions\CAsproMaxCustom::showCalculateDeliveryBlock($arResult['ID'], $arParams);?>
				...
				<div class="main_item_wrapper js-offers-calc product-action">
					<?$frame = $this->createFrame()->begin('');?>
										...
										<?if($arResult["OFFERS"]):?>
											...
											<div class="js_price_wrapper">
												<?if($arCurrentSKU):?>
													...
													<?endif;?>
												<?else:?>
													<div class="price-cont-item">
														<span class="price-cont-item-text">Цена:</span>
							...
							<?$this->SetViewTarget('PRODUCT_SIDE_INFO', 500);?>
								<div class="js-prices-in-side product-action">
									<div class="buy_block">
	                                    ...
	                                    ?>
	                                    <div class="telef">
	                                        <a href="<?=$arRegion["PHONES"][0]["HREF"];?>" class="btn btn-default"><i class="svg  svg-inline-icon" aria-hidden="true"><svg width="18" height="18" viewBox="0 0 18 18" fill="#fff"><path data-name="Shape 915 copy 16" class="" d="M1675.69,597.707l-4.3,4.291H1674a1,1,0,0,1,0,2h-5a1,1,0,0,1-1-1v-5a1,1,0,0,1,2,0v2.543l4.26-4.26A1.01,1.01,0,0,1,1675.69,597.707Zm-8.11,16.264a8.9,8.9,0,0,1-4.36-2.409c-0.35-.367-0.8-0.727-1.21-1.156h0l-0.64-.686h0c-0.4-.435-0.74-0.92-1.09-1.287a9.933,9.933,0,0,1-2.25-4.649,5.628,5.628,0,0,1,1.67-4.627c1.95-1.926,3.6-1.073,4.56-.065a5.46,5.46,0,0,1,1.71,3.785c-0.03,1.845-1.18,2.391-.96,2.584,0.08,0.071.23,0.244,0.4,0.427l0.64,0.685a5.412,5.412,0,0,1,.4.43c0.19,0.236.7-1,2.43-1.025a3.56,3.56,0,0,1,3.09,1.342,3.332,3.332,0,0,1-.06,4.865A4.921,4.921,0,0,1,1667.58,613.971Zm3.33-4.872a2.043,2.043,0,0,0-1.69-1.105,2.271,2.271,0,0,0-2.39.891c-0.66.713-1.25-.121-2.12-1.034l-0.65-.686c-0.85-.935-1.63-1.561-0.97-2.264a2.436,2.436,0,0,0,.94-2.393,4.038,4.038,0,0,0-1.44-2.463,1.665,1.665,0,0,0-1.79.47,3.94,3.94,0,0,0-.8,2.894,7.779,7.779,0,0,0,1.52,3.62c0.23,0.263.63,0.8,1.13,1.32l0.65,0.686c0.49,0.533.99,0.966,1.23,1.206a7.023,7.023,0,0,0,3.4,1.741,3.056,3.056,0,0,0,2.54-.972A1.955,1.955,0,0,0,1670.91,609.1Z" transform="translate(-1658 -596)"></path></svg></i> <?=$arRegion["PHONES"][0]["PHONE"];?></a>
	                                    </div>
									</div>
								</div>
							<?$this->EndViewTarget();?>
						...
						<div class="adaptive-block">
							...
							<?php 
								$minPriceOffer = !empty($arResult['PROPERTIES']['MINIMUM_PRICE']['VALUE']) ? $arResult['PROPERTIES']['MINIMUM_PRICE']['VALUE'] : null;
								if (!$minPriceOffer) {
									$minPriceOffer = ($arResult['MIN_PRICE']['DISCOUNT_VALUE'] ? $arResult['MIN_PRICE']['DISCOUNT_VALUE'] : $arResult['MIN_PRICE']['VALUE']);
								}
							?>

							<div class="" itemprop="offers" itemscope itemtype="http://schema.org/Offer">
								<meta itemprop="price" content="<?=$minPriceOffer;?>" />
	            ...
	            <?//form id ZAMERSHCHIK?>
	            <?$this->SetViewTarget('PRODUCT_SIDE_INFO', 730);?>
	            <div class="muted777 muted ncolor ">
	                <span class="btn btn-default has-ripple ZAMERSHCHIK" data-event="jqm" data-param-form_id="ZAMERSHCHIK" data-name="ZAMERSHCHIK">Вызвать замерщика</span>
	            </div>
	            <?$this->EndViewTarget();?>
		...
		<?if($arResult['OFFERS']):?>
			<?php /* ?><span itemprop="offers" itemscope itemtype="http://schema.org/Offer" style="display:none;" data-offer="1">
				<?php 
					$minPriceOffer = !empty($arResult['PROPERTIES']['MINIMUM_PRICE']['VALUE']) ? $arResult['PROPERTIES']['MINIMUM_PRICE']['VALUE'] : null;
				?>
				<?foreach($arResult['OFFERS'] as $arOffer):?>
					...
					<?endforeach;?>
					<?php 
						$minPriceOfferCurrent = ($arOffer['MIN_PRICE']['DISCOUNT_VALUE']) ? $arOffer['MIN_PRICE']['DISCOUNT_VALUE'] : $arOffer['MIN_PRICE']['VALUE'];
						if ($minPriceOffer == $minPriceOfferCurrent): ?>
							<span itemprop="offers" itemscope itemtype="http://schema.org/Offer">
								<meta itemprop="sku" content="<?=implode('/', $currentOffersList)?>" />
								<a href="<?=$arOffer['DETAIL_PAGE_URL']?>" itemprop="url"></a>
								<meta itemprop="price" content="<?=$minPriceOfferCurrent?>" />
...
<?if($arParams["SHOW_HOW_BUY"] == "Y"):?>
    ...
    ?>
    <?
    if($arParams["SECTION_CODE"] == 'betononasosy')
        $APPLICATION->IncludeFile(SITE_DIR."include/tab_catalog_detail_howbuy_arenda.php", array(), array("MODE" => "html", "NAME" => GetMessage('TITLE_HOW_BUY')));
    else
    ...
    ?>
    <?
    $strHowToBuy = ob_get_clean();
    echo str_replace(array("#NAME#", "#SNAME#", "#PRICE#"), array($arResult["NAME"], mb_strtolower($arResult["NAME"]), $arResult["MIN_PRICE"]["PRINT_VALUE_VAT"]), $strHowToBuy);
    ?>	<?$this->EndViewTarget();?>
<?endif;?>
...
<?if($arParams["SHOW_PAYMENT"] == "Y"):?>
	<?$this->SetViewTarget('PRODUCT_PAYMENT_INFO');?>
        ...
        <?
        $out = ob_get_clean();
        echo str_replace(array("#NAME#", "#SNAME#", "#PRICE#"), array($arResult["NAME"], mb_strtolower($arResult["NAME"]), $arResult["MIN_PRICE"]["PRINT_VALUE_VAT"]), $out);
        ?>
    <?$this->EndViewTarget();?>
<?endif;?>
...
<?if($arParams["SHOW_DELIVERY"] == "Y"):?>
    ...
    <?
    if($arParams["SECTION_CODE"] == 'betononasosy')
        $APPLICATION->IncludeFile(SITE_DIR."include/tab_catalog_detail_delivery_arenda.php", array(), array("MODE" => "html", "NAME" => "Условия"));
    else
			...
			<div class="maxwidth-theme only-on-front">
				<div class="item-views tizers left">
					<div class="items small-block  tops">
						<div class="row flexbox  ">
							<?
							$arSelect = Array("ID", "IBLOCK_ID", "NAME", "DATE_ACTIVE_FROM", "PREVIEW_PICTURE", "PROPERTY_ICON", "PROPERTY_LINK", "TIZERS_BLOG"); // тут указываешь поля инфоблока которые нужны
							$arFilter = Array("IBLOCK_ID"=> 10, "ACTIVE"=>"Y", "ID"=> $arResult["PROPERTIES"]["TIZERS"]["VALUE"]);
									...
									<div class="item flexbox flexbox--row">
										<div class="image shine colored_theme_svg pull-left">
											<?if($arFields["PROPERTY_LINK_VALUE"]) {?>
												<a href="<?=$arFields["PROPERTY_LINK_VALUE"]?>">
													<?
														$imageId = $arFields["PROPERTY_ICON_VALUE"]; // ID изображения
														$imageUrl = CFile::GetPath($imageId);
														$imageContent = file_get_contents($_SERVER["DOCUMENT_ROOT"] . $imageUrl);
													?>
													<?=$imageContent;?>	
												</a>
											<?}else {?>
										...
										<div class="inner-text">
											<div class="title font_xs">
												<?if($arFields["PROPERTY_LINK_VALUE"]) {?>
													<a href="<?=$arFields["PROPERTY_LINK_VALUE"]?>">
														<span class="muted777"><?=$arFields["NAME"]?></span>
													</a>
												<?}else {?>
													<div>
														<span class="muted777"><?=$arFields["NAME"]?></span>
