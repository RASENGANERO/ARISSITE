...
if (!empty($arResult['IBLOCK_SECTION_ID'])) $APPLICATION->SetPageProperty('sectionId', $arResult['IBLOCK_SECTION_ID']);
...
<?  $APPLICATION->ShowViewContent('TIZERS_CAT'); ?>
<div class="li_none">
<?if($arResult["ID"]):?>
	<?//tizers?>
...
<?endif;?>
	...
	<?if($templateData['LINK_SALES'] && $arParams['IBLOCK_LINK_SALE_ID']):?>
        ...
        if(!empty($arRegion["ID"])){
            $GLOBALS['arrSalesFilter'] = array_merge(["PROPERTY_LINK_REGION" => $arRegion["ID"]],$GLOBALS['arrSalesFilter']);
        }
        ?>
			<?$APPLICATION->IncludeComponent(
				"bitrix:news.list",
	...
	if($bShowPropsTab && $arParams["PROPERTIES_DISPLAY_LOCATION"] != "TAB")
		--$i;?>
	<?php $isShowTabs = false;?>
	<?foreach($arBlockOrder as $arBlockOrderKey => $code):?>
		<?//complect?>
								...
								<?foreach($arTabOrder as $value):?>
									...
									<?if($value == "desc"):?>
										<?if($bShowDetailTextTab || ($arParams["PROPERTIES_DISPLAY_LOCATION"] != "TAB" && $bShowPropsTab)):?>
											<li data-tid="b1" class="bordered rounded3 <?=(!($iTab++) ? 'active' : '')?>"><a href="#desc" data-toggle="tab"><?=($arParams["T_DESC"] ? $arParams["T_DESC"] : Loc::getMessage("T_DESC"));?></a></li>
										<?endif;?>
									<?endif;?>
									<?//show char block?>
									<?if($value == "char"):?>
										<?if($bShowPropsTab && $arParams["PROPERTIES_DISPLAY_LOCATION"] == "TAB"):?>
											<li data-tid="b2" class="bordered rounded3 <?=(!($iTab++) ? 'active' : '')?>"><a href="#props" data-toggle="tab"><?=($arParams["T_CHARACTERISTICS"] ? $arParams["T_CHARACTERISTICS"] : Loc::getMessage("T_CHARACTERISTICS"));?></a></li>
										<?endif;?>
									<?endif;?>
									<?//show howbuy block?>
									<?if($value == "buy"):?>
										<?if($bShowHowBuyTab):?>
											<li class="bordered rounded3 <?=(!($iTab++) ? 'active' : '')?>"><a href="#buy" data-toggle="tab"><?if($arParams["SECTION_CODE"] == 'betononasosy') {echo "Как арендовать";}else{?><?=$arParams["TITLE_HOW_BUY"];?><?}?></a></li>
										<?endif;?>
									<?endif;?>
									<?//show payment block?>
									<?if($value == "payment"):?>
                                    ...
                                    <?if($value == "delivery"):?>
                                        <?if($bShowDeliveryTab):
                                            if($arParams["SECTION_CODE"] == 'betononasosy') {?>
                                                <li class="bordered rounded3 <?=(!($iTab++) ? 'active' : '')?>"><a href="#delivery" data-toggle="tab">Условия</a></li>
                                                <?
                                            } else {
                                                ?>
                                        ...
                                        <li class="bordered rounded3 <?///=(!($iTab++) ? 'active' : '')?>"><a href="#servicesbl" data-toggle="tab">Услуги</a></li>
                                    <?endif;?>
									<?//show stores block?>
									<?if($value == "stores"):?>
										<?if($bShowStoresTab):?>
									...
									<?if($value == "reviews"):?>
										<?if($bShowReviewsTab):?>
											<li data-tid="b9" class="bordered rounded3 <?=(!($iTab++) ? 'active' : '')?>">
                            ...
                            <?if($value == "delivery"):?>
                                <?if($bShowDeliveryTab):?>
                                    <div class="tab-pane <?=$value;?> <?=(!($iTab++) ? 'active' : '')?>" id="delivery">
                                        ...
                                        <?if($arParams["SECTION_CODE"] != 'betononasosy' && $arParams['CALCULATE_DELIVERY'] != 'NOT' && (!$arResult["OFFERS"] || $arParams['TYPE_SKU'] === 'TYPE_1') ):?>
                                ...
                                <div class="tab-pane servicesbl <?///=(!($iTab++) ? 'active' : '')?>" id="servicesbl">
                                    ...
                                    <?$APPLICATION->IncludeComponent(
                                        ...
                                        array(
                                            ...
                                            "PARENT_SECTION_CODE" => "",
                                            "INCLUDE_SUBSECTIONS" => "Y",
                                            "PAGER_TEMPLATE" => "services_no_canonical",
                                            "DISPLAY_TOP_PAGER" => "N",
                                            "DISPLAY_BOTTOM_PAGER" => "N",
                                            "PAGER_TITLE" => "Новости",
                                            "PAGER_SHOW_ALWAYS" => "N",
                                    ...
                                    <?$APPLICATION->ShowViewContent('PRODUCT_SERVICESB')?>
                                </div>
                            <?endif;?>
							<?//custom_tab?>
							<?if($value == "custom_tab"):?>
		...
		<?php if ($isShowTabs): ?>
			...
			<div data-tid="b4" class="ordered-block__title option-font-bold font_lg">Декларации ГОСТ</div>
            <?
            $APPLICATION->IncludeComponent(
                "bitrix:news.list",
                "slider_sertificate",
            ...
            <?php
            	$CATALOG_SECTION_ID = $arResult['IBLOCK_SECTION_ID'];
            	if (!empty($arResult['SECTION']['ID'])) {
					$CATALOG_SECTION_ID = $arResult['SECTION']['ID'];
            	}
            ?>
            <div data-tid="b7"></div>
            <?$APPLICATION->IncludeComponent(
				"bitrix:news", 
				"faq_on_element_detail", 
				array(
					"ADD_ELEMENT_CHAIN" => "N",
					"ADD_SECTIONS_CHAIN" => "N",
					"AJAX_MODE" => "N",
					...
					"CACHE_TYPE" => "A",
					"CHECK_DATES" => "Y",
					"COMPONENT_TEMPLATE" => "faq_on_element_detail",
					"COMPOSITE_FRAME_MODE" => "A",
					"COMPOSITE_FRAME_TYPE" => "AUTO",
					"COUNT_IN_LINE" => "3",
					...
					"FAQ_SECTIONS" => $arResult['FAQ_SECTIONS'],
					"CLASS" => "mb-3",
					"SHOW_TITLE_FAQ" => "Y",
...
if(!empty($arResult["DIAGRAMMA_ID"]) && !empty($arResult["DIAGRAMM_CATEGORY"]) && !empty($arResult["DIAGRAMM_VALUES"]) && !empty($arResult["DIAGRAMM_NAME"])) {

    $APPLICATION->IncludeComponent(
        "ithive:amchartscomponent",
        "incatalog_new",
        array(
            "CATEGORY_AXIS_NAME" => "Категории",
            "CATEGORY_AXIS_VALUES" => $arResult["DIAGRAMM_CATEGORY"],
            "CHART_NAME" => $arResult["DIAGRAMM_NAME"],
            "CHART_TYPE" => "pie",
            "CHART_WIDTH" => "100%",
            "COLUMN_COLOR" => "red",
            "VALUE_AXIS_NAME" => "Значения",
            "VALUE_AXIS_VALUES" => $arResult["DIAGRAMM_VALUES"],
            "CHART_ID" => $arResult["DIAGRAMMA_ID"]
        ),
        false
    );
}
...
<?php
if (!empty($arResult['NOINDEX']) && $arResult['NOINDEX'] === 'Y') {
	$APPLICATION->SetPageProperty("SET_NOINDEX", true);
}

$APPLICATION->SetPageProperty("not_extends_parent", true);
?>