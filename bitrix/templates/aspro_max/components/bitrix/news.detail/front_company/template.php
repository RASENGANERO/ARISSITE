...
<div class="content_wrapper_block <?=$templateName;?>">
	<div class="maxwidth-theme <?=$arParams['TYPE_BLOCK'] == 'type2' ? '' : 'wide'?> ">
		...
		if($bShowImage)
		{
            ...
            $arSelect = Array("ID", "NAME", "DATE_ACTIVE_FROM", "PROPERTY_REGION_IMAGE_INDEX");
            $arFilter = Array("IBLOCK_ID" => 4, "ACTIVE" => "Y", "ID" => $GLOBALS["arRegionLink"]);
            $res = CIBlockElement::GetList(Array(), $arFilter, false, false, $arSelect);
            if($ob = $res->GetNextElement()){
                $arFields = $ob->GetFields();
                if($arFields["PROPERTY_REGION_IMAGE_INDEX_VALUE"]){
                    $regionFileSrc = CFile::GetFileArray($arFields["PROPERTY_REGION_IMAGE_INDEX_VALUE"]);

//                    echo "<pre>"; print_r($regionFileSrc); echo "</pre>";
                    $imageSrc = $regionFileSrc["SRC"];
                    $imageHeight = $regionFileSrc["HEIGHT"];
                    $imageWidth = $regionFileSrc["WIDTH"];
                    $imageDescription = $regionFileSrc["DESCRIPTION"];
                }
            }
		...
		<div class="item-views company lazy <?=$lightText ? 'company_light_text' : '';?> <?=$arParams['TYPE_IMG'];?> <?=$arParams['TYPE_BLOCK'];?><?=($arParams['TYPE_IMG'] == 'lg' ? ' ' : ' with-padding');?>" <?if($arResult['BG_IMG']):?>data-src="<?=$arResult['BG_IMG'];?>" style="background-image:url(<?=\Aspro\Functions\CAsproMax::showBlankImg($arResult['BG_IMG']);?>)"<?endif;?>>
			<div class="company-block">
				<div class="row w100 flexbox<?=($arParams['TYPE_BLOCK'] == 'type2' ? ($arParams['REVERCE_IMG_BLOCK'] == 'Y' ? '' : ' flex-direction-row-reverse') : ($arParams['REVERCE_IMG_BLOCK'] == 'Y' ? ' flex-direction-row-reverse' : ''))?>">
					...
					<div class="col-md-<?=($arParams['TYPE_BLOCK'] == 'type2' ? ($arParams['TYPE_IMG'] == 'md' ? 4 : 3) : 6);?> image-block col-xs-12">
						<div class="item video-block">
							...
							<?elseif($bImage):?>
								<div alt="<?=$imageDescription;?>" title="<?=$imageDescription;?>" class="image lazy<?=($arParams['TYPE_BLOCK'] == 'type2' ? ' rounded' : '');?>" data-src="<?=$imageSrc;?>" style="background-image:url(<?=\Aspro\Functions\CAsproMax::showBlankImg($imageSrc);?>)"  data-video_source="<?=$videoSource?>" <?=(strlen($videoPlayer) ? ' data-video_player="'.$videoPlayer.'"' : '')?><?=(strlen($videoPlayerSrc) ? ' data-video_src="'.$videoPlayerSrc.'"' : '')?>>
                                    <?
                                    if(!empty($imageHeight) && !empty($imageWidth)){?>
                                        <script>
                                            if($(window).width() > 768){
                                                ...
                                                var boxWidth = $(".company-block .image").width();
                                                var newHeight = Math.round(((boxWidth*imageHeight)/imageWidth));
                                                $(".company-block .image").height(newHeight);
                                                $(".company-block .image").css("background-size", "contain");
                                            }
                                        </script>
                                    <?}
                                    ?>
