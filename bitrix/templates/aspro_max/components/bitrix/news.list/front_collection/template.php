							...
							<?if($imageSrc):?>
								<div class="image pattern <?=(!$bBgImg ? 'shine' : '');?>">
									<div class="wrap">
										...
										<span itemscope itemtype="https://schema.org/ImageObject" class="img-custom-1-wrapper bg-fon-img rounded3 <?=($bBgImg ? 'darken-bg-animate' : '');?><?=$position;?>">
											<img class="lazy img-custom-1" data-replace="<?=$arItem['FIELDS']['DETAIL_PICTURE']["SRC"];?>" src="<?=$arItem['FIELDS']['DETAIL_PICTURE']["SRC"];?>" <?=\Helper\CHelperCommon::getSrcSet($arItem['FIELDS']['DETAIL_PICTURE']['ID'], 225, 150);?> title="<?=$arItem['FIELDS']['DETAIL_PICTURE']['TITLE']?>" alt="<?=$arItem['FIELDS']['DETAIL_PICTURE']['TITLE']?>" decoding="async" itemprop="contentUrl" />
										</span>
