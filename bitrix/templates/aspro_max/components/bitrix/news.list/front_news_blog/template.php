...
<?if($arResult['ITEMS']):?>
	...
	<?if(!$arParams['IS_AJAX']):?>
		...
		<?if($arParams['TITLE_BLOCK'] || $arParams['TITLE_BLOCK_ALL']):?>
			...
			<?endif;?>
		<?endif;?>
		<div itemscope="itemscope" itemtype="https://schema.org/Blog" class="item-views news2 <?=$arParams['TYPE_IMG'];?><?=(!$bBordered ? '' : ' with-border');?><?=($arParams['HALF_BLOCK'] == 'Y' ? ' half-block' : '');?> <?=($bgSmallPlate ? 'small-bg-plate' : '');?> <?=$sTemplateMobile;?>">
	...
	<?endif;?>
			...
			<?foreach($arResult['ITEMS'] as $i => $arItem):?>
				...
				<?if($arParams['HALF_BLOCK'] != 'Y' || ($arParams['HALF_BLOCK'] == 'Y' && $arParams['IS_AJAX'] != 'Y' && $indexItem < 2)):?>
					<div itemscope itemtype="https://schema.org/BlogPosting" itemprop="blogPosts" class="item-wrapper col-md-<?=$col;?> col-sm-6 col-xs-6 col-xxs-12 clearfix <?=$arItem['PROPERTIES']['TYPE_BLOCK']['VALUE_XML_ID'];?> <?=$dop_class;?><?=($bSlider ? ' item-width-261' : '');?>" data-ref="mixitup-target">
				<?endif;?>
					...
					<div class="item <?=$bHalfWrapper ? '' : 'bg-white'?> <?=($bBordered ? ' bordered box-shadow rounded3' : '');?><?=(!$bImage ? ' no-img' : '');?><?=(($arResult['HAS_TITLE_FON'] == 'Y' || $useImageAlternate) ? ' long' : '');?> clearfix" id="<?=$this->GetEditAreaId($arItem['ID']);?>">
						<?if($bImage):?>
							<div class="image shine<?=($bLineImg ? ' pull-left' : '');?>">
								...
								<?endif;?>
									<span itemscope itemtype="https://schema.org/ImageObject" class="img-custom-2-wrapper <?=(!$bBordered ? 'rounded3' : '');?> bg-fon-img <?=$position;?>" style="padding-top: 0;">
										<img itemprop="image" class="lazy img-custom-2" data-replace="<?=$arItem['FIELDS']['DETAIL_PICTURE']["SRC"];?>" src="<?=$arItem['FIELDS']['DETAIL_PICTURE']["SRC"];?>" <?=\Helper\CHelperCommon::getSrcSet($arItem['FIELDS']['DETAIL_PICTURE']['ID'], 480, 330);?> title="<?=$arItem['FIELDS']['DETAIL_PICTURE']['TITLE']?>" alt="<?=$arItem['FIELDS']['DETAIL_PICTURE']['TITLE']?>" decoding="async" itemprop="contentUrl" />
									</span>
								<?if($bDetailLink):?>
									</a>
								<?endif;?>
					...
					<?endif;?>
						<div class="inner-text<?=($bShowSection ? ' with-section' : '');?><?=($bActiveDate ? ' with-date' : '');?><?=($arParams['TITLE_SHOW_FON'] == 'Y' ? ' with-fon' : '');?>">
							...
							<?if($arItem['FIELDS']['PREVIEW_TEXT'] && !$bImage):?>
								<div itemprop="description" class="preview-text muted777 font_xs"><?=$arItem['PREVIEW_TEXT'];?></div>
							<?endif;?>
