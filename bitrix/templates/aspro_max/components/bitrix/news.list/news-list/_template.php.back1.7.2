						...
						<div class="item  noborder<?=($bImage ? '' : ' wti')?><?=($bActiveDate ? ' wdate' : '')?> clearfix" id="<?=$this->GetEditAreaId($arItem['ID'])?>">
							<?if($bImage):?>
								<div itemscope itemtype="https://schema.org/ImageObject" class="image shine nopadding">
									...
									<?endif;?>
										<img data-replace="<?=$imageSrc;?>" src="<?=$imageSrc;?>" alt="<?=($bImage ? $arItem['PREVIEW_PICTURE']['ALT'] : $arItem['NAME'])?>" title="<?=($bImage ? $arItem['PREVIEW_PICTURE']['TITLE'] : $arItem['NAME'])?>" class="img-responsive lazy" data-src="<?=$imageSrc?>" <?php echo \Helper\CHelperCommon::getSrcSet($arItem['PREVIEW_PICTURE']['ID'], 466, 203);?> decoding="async" itemprop="contentUrl" />
									<?if($bDetailLink):?>
										</a>
									<?endif;?>
								</div>
