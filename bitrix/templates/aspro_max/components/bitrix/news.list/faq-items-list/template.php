									...
									<?ob_start();?>
										<?if($bImage || $onlyImgPart):?>
											<div class="image <?=($bImage ? ' w-picture' : ' wo-picture wpi')?>">
												...
												<?endif;?>
													<img data-replace="<?=\Aspro\Functions\CAsproMax::showBlankImg($imageSrc);?>" src="<?=\Aspro\Functions\CAsproMax::showBlankImg($imageSrc);?>" data-src="<?=$imageSrc?>" alt="<?=($bImage ? $arItem['PREVIEW_PICTURE']['ALT'] : $arItem['NAME'])?>" title="<?=($bImage ? $arItem['PREVIEW_PICTURE']['TITLE'] : $arItem['NAME'])?>" class="img-responsive lazy" />
												<?if($bDetailLink):?>
													</a>
												<?elseif(isset($arItem['FIELDS']['DETAIL_PICTURE'])):?>
