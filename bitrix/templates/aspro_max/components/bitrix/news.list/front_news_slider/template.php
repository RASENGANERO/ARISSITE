...
<?if($arResult['ITEMS']):?>
	...
	<?if(!$arParams['IS_AJAX']):?>
		<?php if (isset($arParams['BG_GRAY']) && $arParams['BG_GRAY'] == 'Y'):?>
			<div class="drag-block grey grey_block landings_list-on-front">
		<?php endif;?> 
		<div class="content_wrapper_block swipeignore <?=$templateName;?> content_news2 <?=$arResult['NAV_STRING'] ? '' : 'without-border'?>">
		...
		<?if($arParams['TITLE_BLOCK'] || $arParams['TITLE_BLOCK_ALL']):?>
			...
			<?else:?>
				<div class="top_block clearfix">
					<h2 class="pull-left font_lg"><?=$arParams['TITLE_BLOCK'];?></h2>
					<?if($arParams['SHOW_SUBSCRIBE'] == 'Y' && $arParams['TITLE_SUBSCRIBE']):?>
	...
	<?endif;?>
			...
			<?foreach($arResult['ITEMS'] as $i => $arItem):?>
				...
				<?if($arParams['HALF_BLOCK'] != 'Y' || ($arParams['HALF_BLOCK'] == 'Y' && $arParams['IS_AJAX'] != 'Y' && $indexItem < 2)):?>
					<div class="item-wrapper clearfix <?=$arItem['PROPERTIES']['TYPE_BLOCK']['VALUE_XML_ID'];?> <?=$dop_class;?><?=($bSlider ? ' item-width-261' : '');?>" data-ref="mixitup-target">
				<?endif;?>
							...
							<div class="image shine<?=($bLineImg ? ' pull-left' : '');?>">
								...
								<?endif;?>
									<span itemscope itemtype="https://schema.org/ImageObject" class="img-custom-2-wrapper <?=(!$bBordered ? 'rounded3' : '');?> bg-fon-img <?=$position;?>" style="padding-top: 0;">
										<img class="lazy img-custom-2" data-replace="<?=$arItem['FIELDS']['DETAIL_PICTURE']["SRC"];?>" src="<?=$arItem['FIELDS']['DETAIL_PICTURE']["SRC"];?>" <?=\Helper\CHelperCommon::getSrcSet($arItem['FIELDS']['DETAIL_PICTURE']['ID'], 480, 330);?> title="<?=$arItem['FIELDS']['DETAIL_PICTURE']['TITLE']?>" alt="<?=$arItem['FIELDS']['DETAIL_PICTURE']['TITLE']?>" decoding="async" itemprop="contentUrl" />
									</span>
								<?if($bDetailLink):?>
									</a>
								<?endif;?>
