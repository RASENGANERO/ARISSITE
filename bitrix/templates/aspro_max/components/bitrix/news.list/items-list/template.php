...
<?if($arResult['SECTIONS']):?>
	...
	<?endif;?>
	<?if($arParams['SHOW_TITLE_FAQ'] == 'Y'):?>
		<h2 class="ordered-block__title option-font-bold font_lg ordered-block__title-custom"><?=$arParams["T_TITLE_FAQ"];?></h2>
	<?endif;?>
<div class="item-views items-list1 <?=($onlyImgPart ? 'only-img' : '')?> <?=($documentsMode ? 'documents-mode' : '');?> <?=($licensesMode ? 'licenses-mode' : '');?> <?=$arParams['VIEW_TYPE']?> <?=$arParams['VIEW_TYPE']?>-type-block <?=($arParams['SHOW_TABS'] == 'Y' ? 'with_tabs' : '')?> <?=($arParams['IMAGE_POSITION'] ? 'image_'.$arParams['IMAGE_POSITION'] : '')?> <?=($templateName = $component->{'__parent'}->{'__template'}->{'__name'})?> <?=(!empty($arParams['CLASS'])) ? $arParams['CLASS'] : '' ?>">
	<?// top pagination?>
						...
						<div id="<?=$this->GetEditAreaId($arSection['ID'])?>" class="tab-pane <?=(!$si++ || !$arSection['ID'] ? 'active' : '')?>">
							...
							<?endif;?>
								<?foreach($arSection['ITEMS'] as $i => $arItem):?>
									...
									<?ob_start();?>
										<?if($bImage || $onlyImgPart):?>
											<div class="image <?=($bImage ? ' w-picture' : ' wo-picture wpi')?>">
												<?if($bDetailLink || !empty($arItem['PROPERTIES']['CUSTOM_LINK']['~VALUE'])):?>
													<a href="<?=(!empty($arItem['PROPERTIES']['CUSTOM_LINK']['~VALUE'])) ? $arItem['PROPERTIES']['CUSTOM_LINK']['~VALUE'] : $arItem['DETAIL_PAGE_URL']?>">
														...
														<?elseif($onlyImgPart):?>
															...
															<?if(strlen($arItem['FIELDS']['NAME'])):?>
																<div class="title font_upper muted">
																	<?if($bDetailLink):?><a href="<?=(!empty($arItem['PROPERTIES']['CUSTOM_LINK']['~VALUE'])) ? $arItem['PROPERTIES']['CUSTOM_LINK']['~VALUE'] : $arItem['DETAIL_PAGE_URL']?>"><?endif;?>
																		<?=$arItem['NAME']?>
																	<?if($bDetailLink):?></a><?endif;?>
																</div>
															<?endif;?>
									...
									<?elseif($arParams['VIEW_TYPE'] == 'accordion-old'):?>
